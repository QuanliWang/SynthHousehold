function data_to_check = GenerateData2Check(phi,w, pi, d, lambda1,...
    lambda2,current_number_of_generation, howmany,hh_size)
    L = size(w,2);
    p = length(d);
    data_to_check = zeros(howmany,8*hh_size+1+hh_size);
    
    %no need to normalize it as it is cared in randomsample_new function
    hhindexh = randomsample_new(pi.*lambda2(:,hh_size-1),rand(howmany,1));
    ...
    for i = 1:length(levelhh)
        ...
        data_to_check(hhindexh == levelhh(i),(1:hh_size)*8) = ...
            repmat(randomsample_new(lambda1(levelhh(i),:),rand(cluster_count(i),1)), 1,hh_size);
