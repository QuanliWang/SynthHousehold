function [hh_size_new,trueextras,data_full_all,z_HH_extra,HHdata1_all,HHdata2_all] = ...
    ...
    z_HH_extra = [HouseHolds2(:,2 * 8 + 1); HouseHolds3(:,3 * 8 + 1); HouseHolds4(:,4*8+1)];
    HHdata1_all = [HHdataorig(:,1);trueextras_ownership];
    HHdata2_all = [HHdataorig(:,2)-1;ones(hh_size_new(2),1);2*ones(hh_size_new(3),1);3*ones(hh_size_new(4),1)];
end
