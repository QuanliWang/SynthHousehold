...
GetHouseData <- function(CombinedData){
  House <- CombinedData[!duplicated(CombinedData$Hhindex),
                        c("ownership","headsex","headrace","headhisp","headage")]
  House <- cbind(as.data.frame(table(CombinedData$Hhindex))$Freq,House)
  House <- cbind(House,1) #Add relate=1 for household head
  House <- as.data.frame(House)
  colnames(House) <- c("HHSize","Owner","HHGender","HHRace","HHHisp","HHAge","HHRelate")
  return(House)
}
