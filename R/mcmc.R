for (i in 1:mc$nrun) {
  ...
  if (is.element(i,synindex)) {
    ...
    synData[[which(synindex ==i)]] <- forsynData$synIndividuals_all[1:dim(forsynData$IndividualData_extra)[1],]
  }
  ...
  #re-combine data and indicators into lists for weighting
  ...
  Pi <- UpdatePiWeighted(para$alpha,para$z_HH_all,hyper$K,struc_weight)
  para$pi <- Pi$pi
  para$u <- Pi$u
